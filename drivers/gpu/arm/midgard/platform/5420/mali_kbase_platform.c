...
#include <platform/5420/mali_kbase_platform.h>
#include <platform/5420/mali_kbase_dvfs.h>
...
static int kbase_platform_power_clock_init(struct kbase_device *kbdev)
{
	...
	clk_set_rate(fout_vpll, MALI_MIDGARD_DEFAULT_CLOCK);
	if (IS_ERR(fout_vpll)) {
		// KBASE_DEBUG_PRINT_ERROR(KBASE_CORE, "failed to clk_set_rate [fout_vpll] = %d\n", MALI_MIDGARD_DEFAULT_CLOCK);
		goto out;
	}
	clk_set_rate(aclk_g3d_dout, MALI_MIDGARD_DEFAULT_CLOCK);
	if (IS_ERR(aclk_g3d_dout)) {
